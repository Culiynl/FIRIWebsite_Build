...
interface ProjectIdea {
  ...
  timeline?: string;
  isFavorited?: boolean; // For local state tracking
...
const FAVORITED_IDEAS_KEY = 'firi_favorited_ideas';
...
function getFavoritedIdeas(): ProjectIdea[] {
    try {
        const stored = localStorage.getItem(FAVORITED_IDEAS_KEY);
        return stored ? JSON.parse(stored) : [];
    } catch (e) {
        console.error("Failed to parse favorited ideas from localStorage", e);
        return [];
    }
}

function toggleFavoriteIdea(ideaToToggle: ProjectIdea) {
    const favorites = getFavoritedIdeas();
    const existingIndex = favorites.findIndex(fav => fav.localId === ideaToToggle.localId);
    let newFavorites: ProjectIdea[];

    if (existingIndex > -1) {
        // Remove from favorites
        newFavorites = favorites.filter(fav => fav.localId !== ideaToToggle.localId);
    } else {
        // Add to favorites
        newFavorites = [ideaToToggle, ...favorites];
    }
    
    localStorage.setItem(FAVORITED_IDEAS_KEY, JSON.stringify(newFavorites));
    
    // Update state for both generated projects (to update the star) and the favorites list
    setState(prevState => ({
        favoritedIdeas: newFavorites,
        generatedProjects: prevState.generatedProjects.map(p => 
            p.localId === ideaToToggle.localId ? { ...p, isFavorited: !p.isFavorited } : p
        )
    }));
}
...
function handleSignOut() {
  if (state.isGuest) {
      startApp(); // Just reset the app state to the login screen
      return;
  }
...
async function fetchDashboardData(userId: string) {
  try {
    ...
    const q = projectsCollectionRef.orderBy("createdAt", "desc").limit(5);
    ...
    const projectCountSnapshot = await projectsCollectionRef.get();
    const totalProjects = projectCountSnapshot.size;
...
async function handleDeleteProject(event: Event) {
    ...
    const projectTitle = state.recentProjects.find(p => p.id === projectId)?.title || "this project";
    ...
    const projectRow = document.querySelector(`li[data-project-id="${projectId}"]`) as HTMLLIElement;
    if(projectRow) projectRow.style.opacity = '0.5';

    try {
        const projectDocRef = db.collection("users").doc(state.user.uid).collection("projects").doc(projectId);
        await projectDocRef.delete();

        const updatedProjects = state.recentProjects.filter(p => p.id !== projectId);
        ...
        if(projectRow) projectRow.style.opacity = '1';
...
function renderDashboard() {
  ...
  return `
    ...
    <div class="dashboard-columns">
        ...
        <div class="card favorited-ideas-list">
             <h3>Favorited Ideas</h3>
             ${state.favoritedIdeas.length > 0 ? `
                <ul>
                  ${state.favoritedIdeas.map(p => `
                    <li data-local-id="${p.localId}">
                      <a href="#" class="favorited-idea-link" data-local-id="${p.localId}">${p.title}</a>
                    </li>`).join('')}
                </ul>
             ` : `<p class="placeholder-text">Ideas you favorite will be saved here for quick access.</p>`}
        </div>
    </div>
        ...
        case "judge":
            return `
              <div class="chat-column">
                  ...
                  <form id="chat-form">
                      <input type="text" id="chat-input" placeholder="${hasNoTokens ? 'Add tokens to chat' : 'Type your answer...'}" required ${hasNoTokens ? 'disabled' : ''}/>
...
function renderResults() {
    ...
    return `
        ...
        <div class="feature-notice" style="margin-bottom: 1.5rem;">These ideas are temporary. Select one to develop it into a full project and save it. Favorite an idea to save it for later.</div>
        ${state.error ? `<div class="error-message" style="margin-bottom: 1.5rem;">${state.error}</div>` : ""}
    ...
    return `
        <div class="project-idea-card card">
            <div class="idea-card-header">
                <h3>${project.title}</h3>
                <button class="favorite-btn ${project.isFavorited ? 'favorited' : ''}" data-local-id="${project.localId}" aria-label="Favorite this idea">
                    ${ICON_FAVORITE}
                </button>
            </div>
        ...
        <div class="project-workspace-layout">
            <div class="project-main-col resizable-panel">
                <details class="card" open>
                    <summary><h3>Project Scorecard</h3></summary>
                 ...
                 <details class="card">
                    <summary><h3>Field Analysis</h3></summary>
                    <div class="content-area details-content">${analysis}</div>
                </details>
                <details class="card">
                    <summary><h3>Recommended Timeline & Next Steps</h3></summary>
                 ...
                 <details class="card">
                    <summary><h3>Initial Resources</h3></summary>
    ...
    document.querySelectorAll(".nav-btn").forEach(button => {
        button.addEventListener("click", (e) => {
            ...
            if (state.view === 'project' || (state.view === 'tools' && state.activeTool === 'judge')) {
                 setState({ view, error: null, chat: null, chatHistory: [] });
            } else {
...
function addEventListenersForView() {
    ...
    switch (state.view) {
        case "dashboard":
            ...
            document.querySelectorAll(".favorited-idea-link").forEach(link => {
                link.addEventListener("click", handleLoadFavoriteIdea);
            });
            ...
            document.querySelectorAll(".tab-btn").forEach(button => {
                button.addEventListener("click", (e) => {
                    ...
                    if(state.activeTool === 'judge' || tool === 'judge') {
                        setState({ activeTool: tool, toolOutput: "", toolInput: {}, chat: null, chatHistory: [] });
                    } else {
        ...
        case "project":
            document.getElementById("back-nav-btn")?.addEventListener("click", () => {
                if (state.projectSource === 'ideation') {
                    ...
                    setState({ view: "results", selectedProject: null, timeline: null, chat: null, chatHistory: [], error: null });
                } else {
            ...
            // Generate timeline if it's missing (for older projects)
            if (state.selectedProject && !state.timeline) {
...
function handleLoadProject(event: Event) {
    ...
    const projectToLoad = state.recentProjects.find(p => p.id === projectId);
    ...
    if (projectToLoad) {
        setState({
            ...
            isLoading: false,
            timeline: projectToLoad.timeline || null, // Set timeline if it exists
            chat: null, // Will be initialized
...
function handleLoadFavoriteIdea(event: Event) {
    event.preventDefault();
    const localId = (event.currentTarget as HTMLElement).dataset.localId;
    if (!localId) return;

    const projectToLoad = state.favoritedIdeas.find(p => p.localId === localId);

    if (projectToLoad) {
        // This flow is similar to selecting a new project, but it uses the favorite as a base
        setState({
            view: 'project',
            selectedProject: projectToLoad,
            projectSource: 'dashboard', // Treat as coming from dashboard
            isLoading: false,
            timeline: null, // Timeline will need to be generated if they save it
            chat: null, 
            chatHistory: [], 
            error: null,
        });
    } else {
        setState({ error: "Could not find the selected favorited idea." });
    }
}
...
For each project idea, provide the following information in a JSON object. Return a JSON array of these objects.`;
        ...
        const schema = {
            ...
            items: {
              ...
              properties: {
                ...
                description: { type: Type.STRING, description: "A concise, one-sentence summary of the project's goal and methodology." },
        ...
        const favorites = getFavoritedIdeas();
        const generatedProjects: ProjectIdea[] = JSON.parse(projectResponse.text).map((p: ProjectIdea, i: number) => {
            const localId = `${Date.now()}-${i}`;
            return {
                ...p,
                localId,
                isFavorited: favorites.some(fav => fav.title === p.title && fav.description === p.description) // A simple check
            }
        });
...
async function handleSelectProject(event: Event) {
    ...
    try {
        ...
        delete projectData.isFavorited;
        ...
        setState(prevState => {
            const updatedRecentProjects = [finalProject, ...prevState.recentProjects]
                .slice(0, 5);
            return {
...
function handleToggleFavorite(event: Event) {
    ...
    const ideaToToggle = state.generatedProjects.find(p => p.localId === localId);
    if (ideaToToggle) {
        toggleFavoriteIdea(ideaToToggle);
    }
}
...
async function initializeProjectWorkspace() {
    ...
    const coachChat = ai.chats.create({
        model: "gemini-2.5-flash",
        config: {
            systemInstruction: `You are an encouraging and knowledgeable AI Research Coach for a student. Your goal is to guide them through their science project from ideation to completion. Be supportive, ask clarifying questions, help them break down complex tasks, and suggest resources. Never give them the answers directly. Instead, empower them to find the answers themselves.

Start the conversation by congratulating them on choosing a project and asking them what their first thought or question is about it.

Sample guiding questions you can use later in the conversation:
- "That's an interesting starting point. What have you discovered so far in your background reading about that?"
- "Can you walk me through your proposed experimental setup? What are your variables?"
- "What do you think is the biggest risk or challenge for this project, and how can we plan for it?"
- "How will you measure the outcome? What does 'success' look like for this experiment?"`
        }
    });
...
The timeline should be broken down into logical phases (e.g., Phase 1: Background Research, Phase 2: Experimentation). Each phase should have a list of specific, actionable tasks. Format the entire output as a single block of HTML using <h3> for phases and <ul>/<li> for tasks. Do not include any other text or markdown characters like \`\`\`html.`;
    ...
    try {
        ...
        return `<div class="error-message">Could not generate a timeline. The AI model may be temporarily unavailable.</div>`;
    }
}
...
async function generateAndSaveTimelineForProject(project: ProjectIdea) {
    if (!project.id || !state.user) return;
    
    // Show a loader in the timeline area
    setState({ timeline: `<div class="loader-container"><div class="loader"></div><p>Generating and saving research plan...</p></div>` });
...
function startApp() {
  ...
  const initialFavorites = getFavoritedIdeas();
  ...
  auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        setState({ authStatus: "signedIn", user, view: 'dashboard', isGuest: false, error: null, tokens: tokenCount, recentQueries: initialQueries, favoritedIdeas: initialFavorites });
        await fetchDashboardData(user.uid);
    } else {
      resetState();
      setState({ authStatus: "signedOut", recentQueries: initialQueries, favoritedIdeas: initialFavorites });
    }
  });
}
