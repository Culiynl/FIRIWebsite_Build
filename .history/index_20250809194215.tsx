...
async function fetchDashboardData(userId: string) {
  try {
    ...
    const recentQuery = projectsCollectionRef.orderBy("createdAt", "desc").limit(5);
    ...
    // Fetch favorited projects
    const favsQuery = projectsCollectionRef.where("isFavorited", "==", true).orderBy("createdAt", "desc");
    const favsSnapshot = await favsQuery.get();
    const favoritedProjects: ProjectIdea[] = [];
    favsSnapshot.forEach(doc => {
        favoritedProjects.push({ id: doc.id, ...doc.data() } as ProjectIdea);
    });
