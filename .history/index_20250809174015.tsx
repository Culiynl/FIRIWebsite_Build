...
type View = "dashboard" | "guides" | "tools" | "results" | "project" | "upgrade";
...
interface AppState {
  ...
  user: User | null; // Changed from firebase.User
  isGuest: boolean;
  view: View;
  isLoading: boolean;
  error: string | null;
...
function handleGuestLogin() {
    const mockUser: User = { // Changed from firebase.User
        uid: 'guest-user',
...
function handleSignOut() {
  ...
  signOut(auth).catch(error => { // Changed from auth.signOut()
...
async function fetchDashboardData(userId: string) {
  try {
    ...
    const q = query(projectsCollectionRef, orderBy("createdAt", "desc"), limit(5));
    ...
    const projectCountSnapshot = await getDocs(projectsCollectionRef);
...
async function consumeToken(): Promise<boolean> {
    if (state.tokens <= 0) {
        if (!state.isGuest) {
            setState({ view: 'upgrade', error: 'You are out of Research Tokens.', isLoading: false, isToolLoading: false, isIdeating: false });
        } else {
...
async function handleUpgrade() {
    ...
    try {
        await updateDoc(userDocRef, { tokens: LIFETIME_TOKENS });
        setState({
            isLoading: false,
...
function renderSidebar() {
  ...
  return `
    ...
    <div class="user-profile">
      ${!state.isGuest ? `
        <div class="token-counter ${hasLowTokens ? 'low-tokens' : ''}">
            <div class="value">${state.tokens.toLocaleString()}</div>
            <div class="label">Research Tokens</div>
            ${hasLowTokens ? `<button id="add-tokens-btn" data-view="upgrade">Add Tokens</button>` : ''}
        </div>
      ` : ''}
      <div class="user-info">
...
function renderMainContent() {
    switch (state.view) {
        ...
        case "upgrade": return renderUpgradeView();
...
function renderUpgradeView() {
    return `
    <div class="page-header">
        <h1>Add Research Tokens</h1>
        <p>Get a lifetime supply of tokens to power your research with a one-time upgrade.</p>
    </div>
    ${state.error ? `<div class="error-message" style="margin-bottom: 1.5rem;">${state.error}</div>` : ""}
...
function addSidebarEventListeners() {
    ...
    document.querySelectorAll(".nav-btn, #add-tokens-btn").forEach(button => {
...
async function handleSelectProject(event: Event) {
    ...
    try {
        ...
        const collectionRef = collection(db, "users", state.user.uid, "projects");
...
async function generateAndSaveTimelineForProject(project: ProjectIdea) {
    ...
    if (timelineHtml && state.user && project.id) {
        try {
            const projectDocRef = doc(db, "users", state.user.uid, "projects", project.id);
            await updateDoc(projectDocRef, { timeline: timelineHtml });
    ...
    if (user) {
        ...
        if (userDocSnap.exists()) {
            ...
            if (userData && userData.tokens !== undefined) {
                ...
                tokenCount = 25;
                await setDoc(userDocRef, { tokens: tokenCount }, { merge: true });
            }
        } else {
        ...
        setState({ authStatus: "signedIn", user, view: 'dashboard', isGuest: false, error: null, tokens: tokenCount });
