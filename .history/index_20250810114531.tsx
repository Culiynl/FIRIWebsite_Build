...
type View = "dashboard" | "guides" | "tools" | "results" | "project" | "membership" | "admin"; // Added 'admin' view
type AuthStatus = "loading" | "initializing" | "signedIn" | "signedOut";
type AiTool = "abstract" | "title" | "category" | "judge";

// Extended User interface to include cohort and labHours
...
interface AppState {
  ...
  // Admin state
  isAdmin: boolean; // New: Flag for admin mode
  allUsers: User[]; // New: List of all users for admin panel
}
...
// Define admin UIDs - THESE MUST MATCH YOUR FIRESTORE SECURITY RULES
const ADMIN_UIDS = ['kewBJELlZ1Xvqm4jkQIdpGYzuNN2', 'EhJE5wPYFiQ8X68wnkR5AJCNPe82'];
...
function handleGuestLogin() {
    const mockUser = {
        ...
        reload: async () => {},
        toJSON: () => ({}),
        cohort: 'Guest Cohort', // Default for guest
        labHours: 0, // Default for guest
    } as User;
    ...
    setState({
        ...
        isAdmin: false, // Guests are never admin
    });
}
...
async function fetchDashboardData(userId: string) {
  try {
    ...
    // New: If the current user is an admin, fetch all users for the admin panel
    if (state.isAdmin) {
...
// New function to fetch all users for admin panel
    ...
    try {
        ...
        usersSnapshot.forEach(doc => {
            ...
            allUsers.push({ 
                ...
                cohort: userData.cohort || 'N/A', // Provide default if undefined
                labHours: userData.labHours || 0,   // Provide default if undefined
                // Add other necessary user properties if needed by the admin panel
            } as User); // Cast to User
        });
        setState({ allUsers, isLoading: false, error: null });
    } catch (error) {
        console.error("Error fetching all users for admin:", error);
        setState({ isLoading: false, error: `Failed to load all users: ${(error as Error).message}` });
    }
}
  ...
  return `
    ...
    <nav class="sidebar-nav">
      <ul>
        ...
        ${state.isAdmin ? `<li><button class="nav-btn ${state.view === 'admin' ? 'active' : ''}" data-view="admin">Admin Panel</button></li>` : ''}
      </ul>
    </nav>
    <div class="user-profile">
      ...
      ` : ''}
      <div class="user-info">
        <img src="${state.user.photoURL || 'https://api.dicebear.com/8.x/bottts/svg?seed=user&backgroundColor=2a3dbe,43b3ff'}" alt="User avatar" class="user-avatar">
        <div class="user-details">
          <p class="user-name">${state.user.displayName || 'Unknown User'}</p>
          <p class="user-email">${state.user.email || 'No Email Provided'}</p>
          ${state.user.cohort ? `<p class="user-meta">Cohort: ${state.user.cohort}</p>` : ''}
          ${state.user.labHours !== undefined ? `<p class="user-meta">Lab Hours: ${state.user.labHours}</p>` : ''}
        </div>
      </div>
...
function renderMainContent() {
    switch (state.view) {
        ...
        case "admin": return renderAdminPage(); // New admin view
            ...
            return `
              <div class="chat-column">
                  ...
                  <div id="chat-container" aria-live="polite">
                      ${state.chatHistory.length === 0 && !state.isLoading ? // Only show initial judge message if chat history is empty and not loading
                        `<div class="chat-message model">${marked.parse("I will be your mock judge today, following the Regeneron ISEF criteria. To begin, is your project a **Science** project or an **Engineering** project?")}</div>`
                        : ''
                      }
...
function renderAdminPage() {
    ...
    return `
        <div class="page-header">
            <h1>Admin Panel</h1>
            <p>Manage user cohorts and lab hours.</p>
        </div>
        ${state.error ? `<div class="error-message" style="margin-bottom: 1.5rem;">${state.error}</div>` : ""}
        ${state.isLoading && state.view === 'admin' && state.allUsers.length === 0 ? `
            <div class="loader-container"><div class="loader"></div><p>Loading users...</p></div>`
            : `
            <div class="card">
                <h2>All Users</h2>
                        ...
                        <tbody>
                            ${state.allUsers.map(user => `
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="admin-cohort-input" data-user-id="${user.uid}" value="${user.cohort || ''}" placeholder="Enter Cohort" />
                                    </td>
                                    <td>
                                        <input type="number" class="admin-labhours-input" data-user-id="${user.uid}" value="${user.labHours !== undefined ? user.labHours : ''}" min="0" />
                                    </td>
                                    <td>
                                        <button class="admin-update-user-btn" data-user-id="${user.uid}">Update</button>
                                    </td>
                                </tr>
                            `).join('')}
                        </tbody>
...
function addSidebarEventListeners() {
    ...
    document.querySelectorAll(".nav-btn").forEach(button => {
        button.addEventListener("click", (e) => {
            ...
            // If navigating to admin, ensure all users are fetched
            if (view === 'admin' && state.isAdmin && state.allUsers.length === 0) {
    ...
    switch (state.view) {
        ...
        case "tools":
            ...
            if(state.activeTool === 'judge'){
                ...
                // Call initializeJudgeChat only if chat history is empty, and only once.
                // It will set the systemInstruction and prime the chat for the first user message.
                // The display of the first judge message is handled in renderActiveTool.
                if (state.chatHistory.length === 0 && !state.systemInstruction) { 
                  initializeJudgeChat(); 
                }
            }
            break;
        ...
        case "membership":
            ...
            break;
        case "admin": // New: Admin panel event listeners
            if (state.isAdmin) {
                document.querySelectorAll(".admin-update-user-btn").forEach(button => {
                    button.addEventListener("click", handleUpdateUserAdmin);
                });
            }
            break;
    }
...
// New: Handle user update from admin panel
async function handleUpdateUserAdmin(event: Event) {
    const button = event.currentTarget as HTMLButtonElement;
    ...
    const cohortInput = row.querySelector('.admin-cohort-input') as HTMLInputElement;
    const labHoursInput = row.querySelector('.admin-labhours-input') as HTMLInputElement;

    const newCohort = cohortInput.value.trim();
    const newLabHours = parseInt(labHoursInput.value, 10);

    if (isNaN(newLabHours) || newLabHours < 0) {
        alert("Lab Hours must be a non-negative number.");
        return;
    }
    ...
    try {
        ...
        setState(prevState => ({
            allUsers: prevState.allUsers.map(u => 
                u.uid === userId ? { ...u, cohort: newCohort, labHours: newLabHours } as User : u
            ),
            isLoading: false,
            error: null
        }));
        ...
        // If the updated user is the current logged-in user, update their state too
        if (state.user && state.user.uid === userId) {
            setState(prevState => ({
                user: { ...prevState.user!, cohort: newCohort, labHours: newLabHours } as User
            }));
        }

    } catch (error) {
        console.error("Error updating user from admin panel:", error);
        setState({ isLoading: false, error: `Failed to update user: ${(error as Error).message}` });
    }
}
...
function initializeJudgeChat() { // Made async, but now it doesn't need to be since no API call
...
Keep your responses concise. Your role is to question and probe for depth of understanding. Do not give answers or overly praise the student. Start the conversation by asking which project type they have.`;
  ...
  setState({ chatHistory: [], systemInstruction }); // chatHistory is now empty
}
...
async function handleJudgeChatSubmit(event: Event) {
    ...
    // Store current chat history BEFORE adding the new user message for immediate UI update.
    // This `historyForApi` will be used to send to the backend.
    const historyForApi = [...state.chatHistory]; 

    // Create the user message object for local display and the API call's 'message' parameter
    ...
    // Update local chat history for immediate display of the user's message
    input.value = "";
    setState({
        isLoading: true,
        chatHistory: [...state.chatHistory, userMessage], // Add user's message for display
        judgeImage: null, // Clear the image preview
    });
    
    const canProceed = await consumeToken();
    if (!canProceed) {
        // If tokens are exhausted, add a model response indicating that
        setState(prevState => ({ 
            isLoading: false, 
            chatHistory: [...prevState.chatHistory, {role: 'model', content: "I can't respond right now because you are out of Research Tokens."}],
            systemInstruction: state.systemInstruction // Keep system instruction if it was set
        }));
        return;
    }
    ...
    try {
        ...
        const apiPayload: any = {
            history: historyForApi, // Pass the conversational history *excluding* the current user message
            message: messageParts   // The current user message is sent as 'message'
        };

        // If a system instruction is present (meaning it's the very first API call for this session),
        // include it in the payload. It will be cleared after the response.
        ...
        console.error(err);
         setState(prevState => ({
            ...
            systemInstruction: null, // Also clear on error, so subsequent attempts don't re-send it.
        }));
    }
}
...
async function initializeApp() {
  ...
  try {
    // Fetch the Firebase config from the backend// Corrected line
const response = await fetch(BACKEND_URL + '/api/config');
    ...
    auth.onAuthStateChanged(async (user) => {
      if (user) {
          ...
          let tokenCount = 0;
          let userCohort: string | undefined = undefined;
          let userLabHours: number | undefined = undefined;

          if (userDocSnap.exists) {
              const userData = userDocSnap.data();
              if (userData) {
                  tokenCount = userData.tokens !== undefined ? userData.tokens : 25; // Default tokens
                  userCohort = userData.cohort;
                  userLabHours = userData.labHours;
              }
          } else {
              // New user, create their document with initial tokens, cohort, labHours
              tokenCount = 25;
              userCohort = "Innovator"; // Default for new users
              userLabHours = 0; // Default for new users
              await userDocRef.set({
                  ...
                  lastLogin: new Date(),
                  tokens: tokenCount,
                  cohort: userCohort, // Set default cohort
                  labHours: userLabHours // Set default lab hours
              }, { merge: true });
          }

          const currentUserIsAdmin = ADMIN_UIDS.includes(user.uid);

          setState({
              authStatus: "signedIn",
              user: { ...user, cohort: userCohort, labHours: userLabHours } as User, // Cast to User interface
              view: 'dashboard',
              isGuest: false,
              error: null,
              tokens: tokenCount,
              recentQueries: initialQueries,
              isAdmin: currentUserIsAdmin // Set admin status
          });
