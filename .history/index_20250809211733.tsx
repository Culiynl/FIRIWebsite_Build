    ...
    document.querySelectorAll(".nav-btn").forEach(button => {
        button.addEventListener("click", (e) => {
            ...
            if (state.view === 'project' || (state.view === 'tools' && state.activeTool === 'judge')) {
                 setState({ view, error: null, chat: null, chatHistory: [], judgeImage: null, workspacePanelLayout: null });
            } else {
            ...
            document.querySelectorAll(".tab-btn").forEach(button => {
                button.addEventListener("click", (e) => {
                    ...
                    if(state.activeTool === 'judge' || tool === 'judge') {
                        setState({ activeTool: tool, toolOutput: "", toolInput: {}, chat: null, chatHistory: [], judgeImage: null });
                    } else {
...
Keep your responses concise. Your role is to question and probe for depth of understanding. Do not give answers or overly praise the student. Start the conversation by asking which project type they have.`;
                  ...
                  setState({ chat: judgeChat, chatHistory: [{role: 'model', content: "I will be your mock judge today, following the Regeneron ISEF criteria. To begin, is your project a **Science** project or an **Engineering** project?"}] });
                }
            }
            break;
        case "project":
...
Be sure to ask questions that help them think critically about their project, not like dumb questions that they could just Google.
If they don't have enough background knowledge, you can provide a brief overview or suggest resources to help them get started.
Talk like a 15 year old would, but still be professional and knowledgeable. Take a look at some sample messages below to get a feel for the tone:
'Step 2 - Reading Literature
Very good review of the literature!
Good depth in the summaries and very good attention to feasibility. 
I think the research field you are exploring is an interesting and very feasible one, especially focusing on machine learning based approaches.

Step 4 - Research Questions
Continue to research using FIRI Guide Sheet 2 and try to develop questions 1 and 3 from sheet 1 for the upcoming meeting. 
It is very key for you to start thinking about what types of tools you will be needing, and sheet two will help with this significantly as well. 
Keep up the great work!' 
This does not mean use emojis.
...
async function handleJudgeChatSubmit(event: Event) {
    ...
    if ((!message || message.trim() === "") && !imageToSend) return;
    if (!chat) return;
    ...
    try {
        // Construct the message payload with parts
        const parts: Part[] = [];
        if (imageToSend) {
            parts.push({
                inlineData: {
