...
function toggleFavoriteIdea(ideaToToggle: ProjectIdea) {
    ...
    if (existingIndex > -1) {
        ...
        // Add to favorites, including the current field analysis
        const ideaWithAnalysis = { ...ideaToToggle, analysis: state.fieldAnalysis };
        newFavorites = [ideaWithAnalysis, ...favorites];
    }
...
function handleLoadFavoriteIdea(event: Event) {
    ...
    const projectToLoad = getFavoritedIdeas().find(p => p.localId === localId);

    if (projectToLoad) {
        // Backwards compatibility: handle old favorites without analysis.
        if (!projectToLoad.analysis) {
            projectToLoad.analysis = `<div class="placeholder-text">Field analysis was not saved with this favorited idea. To generate it, select this idea from a new brainstorm session.</div>`;
        }
        ...
        setState({
            ...
            isLoading: false,
            // A favorite is an unsaved idea, so timeline is not yet generated.
            timeline: null, 
            chat: null, 
            chatHistory: [], 
            error: null,
...
Be supportive, ask clarifying questions, help them break down complex tasks, and suggest resources. Never give them the answers directly. Instead, empower them to find the answers themselves.`;
    ...
    try {
        ...
        const initialResponse = await coachChat.sendMessage({ message: "Hello, please provide your opening message to the student based on my system instructions." });
