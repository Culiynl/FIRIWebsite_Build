  ...
  return `
    ...
    <div class="card">
      ...
      <form id="ideation-form">
        <input type="text" id="topic-input" name="topic" placeholder="e.g., 'chemistry simulations'" required ${isIdeating ? 'disabled' : ''}/>
        <button type="submit" ${isIdeating ? 'disabled' : ''}>${isIdeating ? 'Generating...' : 'Brainstorm Projects'}</button>
      </form>
    </div>
...
function handleLoadProject(event: Event) {
    ...
    if (projectToLoad) {
        setState({
            ...
            isLoading: false,
            timeline: null, // Will be generated
...
async function handleSelectProject(event: Event) {
    ...
    if (state.isGuest) {
        // For guests, just move to the project view without saving.
        ...
        return; // Skip the saving part
    }
    
    if (!state.user) return; // Should not happen if not guest, but good practice
    ...
    try {
        ...
        // Add the new project to the top of the recent projects list in the state
        const updatedRecentProjects = [finalProject, ...state.recentProjects]
            .slice(0, 5); // Keep the list to a max of 5
...
`;
    ...
    // Step 2: Set loading state and save the captured input to state, preserving it during re-render.
    setState({ isToolLoading: true, toolOutput: "", toolInput: capturedInput });

    try {
        // Step 3: Make the API call using the prompt with the captured values.
        const response = await ai.models.generateContent({ model: "gemini-2.5-flash", contents: prompt });
        const output = state.activeTool === 'abstract' ? response.text : await marked.parse(response.text);
        
        // Keep the input in the state so the user can see what they submitted
...
function startApp() {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      setState({ authStatus: "signedIn", user, view: 'dashboard', isGuest: false, error: null });
