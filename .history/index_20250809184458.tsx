...
function sanitizeAiListContent(text: string): string {
    if (!text) return "";
    // This regex matches any combination of whitespace, », •, *, or - at the beginning of each line (gm flags)
...
function renderDashboard() {
  ...
  return `
    ...
    <div class="card recent-queries-list" style="margin-top: 1.5rem;">
        ...
        ${state.recentQueries.length > 0 ? `
            ...
            </ul>` : `<p style="color: var(--text-secondary); margin-top: 0.5rem;">Your recent brainstorm topics will appear here.</p>`
        }
    </div>
...
function renderProjectView() {
    ...
    if (!state.selectedProject) {
        return `
            ...
            <div class="error-message">
             ...
             <button id="back-to-ideas-btn" style="margin-top: 1.5rem;">Back to Ideas</button>
        `;
    }
    ...
    return `
        ...
        <div class="page-header project-header">
            ...
            <button id="back-to-ideas-btn">Back to Ideas List</button>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="upgrade-view">
                <div class="card upgrade-card">
                    ...
                    <button id="upgrade-btn" ${state.isLoading ? 'disabled' : ''}>
...
function addEventListenersForView() {
    if (state.isUpgradeModalOpen) {
        ...
        document.getElementById("upgrade-btn")?.addEventListener("click", async () => {
            await handleUpgrade();
            if(!state.error) { // Close modal on success
                setState({ isUpgradeModalOpen: false });
            }
        });
    }
    ...
    switch (state.view) {
        ...
        case "tools":
            ...
            break;
        case "project":
            document.getElementById("back-to-ideas-btn")?.addEventListener("click", () => {
                // Go back to the ideas list, not the dashboard
...
Present your response clearly. Use markdown for formatting.`;
        ...
        const cleanedAnalysis = fieldAnalysisText.replace(/»/g, '');
        setState({ fieldAnalysis: await marked.parse(cleanedAnalysis), sources: uniqueSources });
...
For each project idea, provide the following information in a JSON object. Return a JSON array of these objects.`;
        ...
        const projects = JSON.parse(projectResponse.text) as ProjectIdea[];
        // Sanitize the HTML content within each project's resources
        const generatedProjects = projects.map(p => ({
            ...p,
            resourcesHtml: sanitizeAiListContent(p.resourcesHtml)
        }));
...
async function handleSelectProject(event: Event) {
    ...
    setState({ isLoading: true, view: "project", error: null });
...
async function generateProjectTimeline(project: ProjectIdea, isGuest: boolean = false): Promise<string | null> {
    ...
    The timeline should be broken down into logical phases. Format the entire output as a single block of HTML.
    - Use <h3> tags for phase titles (e.g., <h3>Phase 1: Research</h3>).
    - Use <ul> and <li> tags for the tasks within each phase.
    - IMPORTANT: Write ONLY the text content inside the <li> tags. Do not add any bullet point characters like '»' or '-' yourself. The HTML <li> tags will handle the bullet points automatically.
    - Do not include any other text or markdown characters like \`\`\`html.`;
    ...
    try {
        ...
        return sanitizeAiListContent(response.text);
    } catch (error) {
...
function startApp() {
  ...
  auth.onAuthStateChanged(async (user) => {
    if (user) {
      ...
      resetState();
      setState({ authStatus: "signedOut", recentQueries: initialQueries });
    }
  });
}
